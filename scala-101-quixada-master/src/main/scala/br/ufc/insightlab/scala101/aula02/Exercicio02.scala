...
object Exercicio02 extends App{
  ...
  object Operador{
    def conjuntoUnitario(n: Int): Set = x => x == n

    def uniao(s: Set, t: Set): Set = x => s(x) || t(x)

    def interseccao(s: Set, t: Set): Set = x => s(x) && t(x)

    def diferenca(s: Set, t: Set): Set = x => s(x) && !t(x) || !s(x) && t(x)

    def complemento(s: Set): Set = x => !s(x)
  }
